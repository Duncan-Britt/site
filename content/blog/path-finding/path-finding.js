...
class Grid {
  ...
  plot_course() {
    ...
    while (pq_size != 0) {
      ...
      for (let i = 0; i < 8; i += 1) {
        // console.log(`this._grid[explored[explored_size - 1].grid_idx + neighbors[i]]: ${this._grid[explored[explored_size - 1].grid_idx + neighbors[i]]}`);
        ...
        // console.log(`this._grid[neighbor_idx]: ${this._grid[neighbor_idx]}`);
...
class CanvasDisplay {
  constructor(parent, grid) {
    ...
    let press_handler = ((e) => {
      ...
      let x = Math.floor(e.offsetX / SCALE);
      let y = Math.floor(e.offsetY / SCALE);
      is_erasing = this.grid.at(x, y) == 1 ? true : false;
      let new_value = is_erasing ? 0 : 1;
      this.grid.set_at(new_value, x, y);
      this.draw();
    }).bind(this);
